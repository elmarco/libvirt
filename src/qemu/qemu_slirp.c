                    ...
                    qemuSlirpFeature feature)
{
    ...
    for (i = 0; i < slirp->nfeatures; i++) {
        if (slirp->features[i] == feature)
            return true;
    }
...
qemuSlirpNewForHelper(const char *helper)
{
    ...
    slirp->nfeatures = virJSONValueArraySize(featuresJSON);

    if (VIR_ALLOC_N(slirp->features, slirp->nfeatures) < 0)
        return NULL;
...
void
qemuSlirpStop(virDomainObjPtr vm,
              virQEMUDriverPtr driver,
               ...
               qemuDomainLogContextPtr logCtxt)
{
    ...
    if (qemuSlirpHasFeature(slirp,
        ...
        if (qemuDBusStart(driver, vm, logCtxt) < 0)
            return -1;
    ...
    if (qemuExtDeviceLogCommand(logCtxt, cmd, "slirp") < 0)
        return -1;
